...
const configFactory = require('../scripts-config/webpack.config')
...
// the previous sizes is always empty. To Safe time replaced
// the function call with the result which always comes back
const MOCK_PREVIOUS_FILE_SIZES = {}

const buildStaticAppFiles = async () => {
  copyPublicFolder()
  const buildResults = await build(CONFIG)
...
const logBuildResults = ({ stats, warnings }) => {
  try {
    if (warnings.length) {
...
module.exports = buildStaticAppFiles
